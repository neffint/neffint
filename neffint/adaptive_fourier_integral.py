...
def improve_frequency_range(
    ...
    func: FuncType, 
    interpolation_error_metric: Callable[[np.ndarray, np.ndarray], np.ndarray],
    absolute_integral_tolerance: Union[float, Sequence[float]],
    ...
    """An adaptive algorithm improve the frequency range to a better range to perform Fourier integration on the input func on.
    ...
    * pchip = piecewise cubic hermite interpolating polynomial
    ...
    or given as a sequence of three floats, for the three loops
    :type absolute_integral_tolerance: Union[float, Sequence[float]]
    ...
    :return: A 1D array, the improved frequency range
    :rtype: np.ndarray
    """
    ...
    integral_values = fourier_integral_fixed_sampling(times, frequencies, func_values, pos_inf_correction_term=False, neg_inf_correction_term=False, interpolation="pchip")
    ...
    while integral_absolute_error > abs_lowscan_tolerance and frequencies[0] > 0:
        ...
        new_integral_contributions = fourier_integral_fixed_sampling(times, frequencies[:2], func_values[:2], pos_inf_correction_term=False, neg_inf_correction_term=False, interpolation="pchip")
    ...
    while integral_absolute_error > abs_highscan_tolerance:
        ...
        func_derivative_end = complex_pchip(2*np.pi*frequencies[-3:], func_values[-3:],  2*np.pi*frequencies[-2], derivative_order=1, axis=0)

        new_integral_contributions = (
            fourier_integral_fixed_sampling(times, frequencies[-2:], func_values[-2:], pos_inf_correction_term=True, neg_inf_correction_term=False, interpolation="pchip")
            - _fourier_integral_inf_correction(times=times, omega_end=2*np.pi*frequencies[-2], func_value_end=func_values[-2], func_derivative_end=func_derivative_end)
        )
...
def fourier_integral_adaptive(
    ...
    * pchip = piecewise cubic hermite interpolating polynomial
    ...
    frequencies, func_values = improve_frequency_range(
        ...
        frequency_bound_scan_logstep=frequency_bound_scan_logstep,
        bisection_mode_condition=bisection_mode_condition
    )
